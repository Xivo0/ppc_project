...
def signal_handler_meteo(signum, frame):
    # On utilise le verrou interne de la variable partagée pour la modifier proprement
    with drought_flag.get_lock():
        drought_flag.value = not drought_flag.value
        
    etat = "ACTIVÉE ☀️" if drought_flag.value else "DÉSACTIVÉE 🌧️"
...
def environment_manager(drought_val):
    ...
    etait_en_secheresse = False # Mémoire de l'état précédent
    
    while True:
        est_en_secheresse = drought_val.value
        ...
        try:
            ...
            pass # Pas de message
        if etait_en_secheresse and not est_en_secheresse:
            print("[MANAGER] La pluie revient... L'herbe va repousser bientôt.")
            # ON RÉINITIALISE LE CHRONO ICI
            # Cela force le système à attendre (ex: 2 secondes) avant la prochaine pousse
            last_growth = time.time()

        # B. GESTION DE LA NATURE
        if not est_en_secheresse:
            if time.time() - last_growth > 2.0:
                with mgr_lock:
                    for _ in range(5):
                ...
                last_growth = time.time()
         # On met à jour l'état pour le prochain tour
        etait_en_secheresse = est_en_secheresse
...
if __name__ == "__main__":
    ...
    try:
        # Quand on reçoit SIGUSR1, on lance signal_handler_meteo
        signal.signal(signal.SIGUSR1, signal_handler_meteo)
    except AttributeError:
        print("[ATTENTION] SIGUSR1 non supporté sur ce système (Windows ?).")
    ...
    try:
        ...
        shm_stats = shared_memory.SharedMemory(name=config.SHM_COUNTERS_NAME, create=True, size=config.STATS_SIZE)
    except FileExistsError:
    ...
    stats_view[1] = 0
    my_pid = os.getpid()
    stats_view[config.IDX_PID] = my_pid
    # 3. Création MQ
    try:

...
def environment_manager(drought_val):
    ...
    while True:
        # 1. GESTION DES REPRODUCTIONS VIA MESSAGE QUEUE
        try:
        ...
        if not drought_val.value and (time.time() - last_growth > 2.0):
            with mgr_lock:
                # L'herbe pousse ! On ajoute 10 unités d'herbe.
...
if __name__ == "__main__":
    ...
    # 1. Calcul de la taille de la mémoire (1 case herbe + Proies x 2 + Prédateurs x 2)
    NB_ENTIERS = 1 + (config.MAX_PREY * 2) + (config.MAX_PREDATOR * 2)
    SIZE_IN_BYTES = NB_ENTIERS * 4 # 4 octets par entier

    # 2. Création Mémoire et IPC
    try:
